...
def process_event(cur_row, prev_row):
    ...
    # double
        ...
        if prev_row['Runner1'] == 0 or prev_row['Runner1'] == 1 or prev_row['Runner3'] == 1:
            cur_row.iloc[9:] = update_cols(prev_row, 2, [1 - prob_1sttohome_double, prob_1sttohome_double])
        else:
            cur_row.iloc[9:] = update_cols(prev_row, 2, [1 - prob_2ndto3rd_double, prob_2ndto3rd_double])

    # triple 
    if cur_row['Result'] == 3:
        cur_row.iloc[9:] = update_cols(prev_row, 3)

    # homerun
    if cur_row['Result'] == 4:
        cur_row.iloc[9:] = update_cols(prev_row, 4)
...
def update_cols(prev_row, hit, prob_wts = None):
    ...
    # double
        ...
        # 1b & 3b -- homebydouble
            ...
            r1 = random.choices([3, 2], weights = prob_wts, k = 1)[0]
            3, 2, 0, 1 + runs if r1 == 3 else 2, 0, 0, 2 + runs 
        # 2b & 3b -- 2ndto3rd
    ...
    # triple
        ...
        # 1b or 2b or 3b
        if r1 != 0 and r2 == 0: return 3, 0, 0, 1 + runs
        # 1b & 2b or 1b & 3b or 2b & 3b
        if r1 != 0 and r2 != 0 and r3 == 0: return 3, 0, 0, 2 + runs
        # bases loaded
        if r3 == 1: return 3, 0, 0, 3 + runs
        else: return 'error', 'error', 'error', 'error'
...
for i in range(len(permutations_condensed)):
    ...
    for j in range(1):
        ...
        run_total.loc[j, 1] = skeleton_data_tmp.loc[80, 'Runs']
