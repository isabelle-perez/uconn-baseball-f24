...
#text_results = text_results.apply(lambda x: 0 if x == 'nan' else x)

for i in range(10):
    for j in range(len(text_results)):
        numeric_value = int(text_results.iloc[i, j]) - 1
        text_results.iloc[i, j] = lookup_table.iloc[numeric_value, 1]
        #pass
